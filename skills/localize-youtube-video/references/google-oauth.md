...
If a JSON download is unavailable, `youtube_configure_oauth` also accepts `client_id` and `client_secret`, but the local file path is the preferred privacy-preserving route.
